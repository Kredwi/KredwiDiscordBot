    ...
    async execute(interaction) {
        try {
            ...
            if (interaction.member.user.id == interaction.message.embeds[0].data.footer.text) {
                ...
                .setColor(0xff0000)
                .setThumbnail(data?.user?.image[3]['#text'] ? data?.user?.image[3]['#text'] + '?size=512' : null)
                .setFields(
                ...
                nowplaying && nowplaying['@attr']?.nowplaying ? {name: lang.listening, value: `${nowplaying?.artist['#text']} - ${nowplaying?.name}`} : {name: lang?.listening, value: lang?.NowPlayingNot},
                )
                .setFooter({
